...
// ReadNotifs appends the read notifs to the list of unread notifs, keeping
// the unread notifs as the first notifs in the list of read notifs, then sets
// the resulting list of unread notifs to the field ReadNotifs and nil to the
// field UnreadNotifs of the user.
func (h *handler) ReadNotifs(userId string) error {
	err := h.users.Update(func(tx *bolt.Tx) error {
		usersBucket := tx.Bucket([]byte(usersB))
		if usersBucket == nil {
			return fmt.Errorf("Bucket %s of users not found\n", usersB)
